...
export default function LotDetailsScreen() {
  ...
  if (!lot) {
    return (
      <ScreenContainer>
        <Text style={styles.title}>Lote nao encontrado</Text>
      </ScreenContainer>
    );
  }
  ...
  return (
    <ScreenContainer>
      ...
      <Text style={styles.subtitle}>{lot.crop} • {lot.propertyName}</Text>

      <View style={styles.grid}>
        <StatCard title="Lucro" value={`R$ ${lot.profit.toLocaleString("pt-BR")}`} />
        <StatCard title="Receita" value={`R$ ${lot.revenue.toLocaleString("pt-BR")}`} />
        <StatCard title="Custo" value={`R$ ${lot.cost.toLocaleString("pt-BR")}`} />
        <StatCard title="Producao" value={`${lot.production} ton`} />
      </View>

      <AppButton label="Editar lote" onPress={() => router.push("/(tabs)/lots/new") } variant="secondary" />
    </ScreenContainer>
  );
}
